...
/*   Updated: 2024/04/21 21:35:17 by alafdili         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	check_width(char **map, char *in_line_map, int y_len, int x_len)
{
	int	index;
	int	tmp;
	...
	{
		if (map[index][tmp] != '1')
			err_alert(in_line_map, map, "Map Not Enclose By Walls", 'm');
...
void	enclosed_walls(char *in_line_map, t_map *list)
{
	...
	check_width(list->map, in_line_map, list->map_y, list->map_x - 1);
	list->map_cp = ft_split(in_line_map, '\n');
	if (!list->map_cp)
		err_alert(in_line_map, list->map, "Cannot Split Map Copy", '0');
	_free(&in_line_map, 'p');
}
